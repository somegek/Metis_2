...
# growth is a function which defines yearly growth on the basis of quarterly data on level
growth <- function(x, y){
...
for(eu in countriesEU){
  ...
  marginTmp_gr <- copy(marginTmp)
  marginTmp_gr[, marginMortgages := growth(marginMortgages, TIME)
            ][, marginCorporations := growth(marginCorporations, TIME)]
  assign(paste0("margin",eu), marginTmp)
  assign(paste0("margin_gr_",eu), marginTmp_gr)
  ...
  margin <- get(paste0("margin_gr_",eu))
  macrosDrive <- get(paste0("macros",eu))
  DT <- merge(margin , macrosDrive, by = "TIME")
  DT <- DT[TIME > 2000]
  
  # fit margin time series to macros with 1 and 2 drivers, the 5 best models according to r2 are recorded
  nMargin <- length(margin) - 1
  assign(paste0("models1var", eu), fitMacros(DT,nMargin,length(DT)- nMargin -1,1, nBest = 5))
...
selectModels_NL <- list(marginMortgages = models1varNL[[1]][3,], marginCorporations = models1varNL[[2]][1,])
selectModels_BE <- list(marginMortgages = models1varBE[[1]][2,], marginCorporations = models1varBE[[2]][4,])
selectModels_ES <- list(marginMortgages = models1varES[[1]][3,], marginCorporations = models1varES[[2]][1,])
selectModels_SE <- list(marginMortgages = models2varSE[[1]][4,], marginCorporations = models2varSE[[2]][3,])
selectModels_IT <- list(marginMortgages = models2varIT[[1]][1,], marginCorporations = models2varIT[[2]][2,])
selectModels_DE <- list(marginMortgages = models1varDE[[1]][1,], marginCorporations = models1varDE[[2]][1,])
...
#eu <- "NL"
for (eu in countriesEU) {
  rateScenarios <- c(paste0("margin_gr_", eu, "_Baseline"), paste0("margin_gr_", eu, "_Adverse"),
                     paste0("margin_gr_", eu, "_optimal"), paste0("margin_gr_", eu, "_extreme"))
  for(loanType in names(models2varNL)){
    ...
    lagged0Vars <- grep("lag0|Lag0", drivers, value = T)
    ...
    if(model$factor2 == "NA"){
      ...
      for (scen in scenarios) {
        macros[, paste0("margin_gr_", eu, "_", scen) := model$intercept + model$factor1Coef * macros[ , grep(scen, grep(gsub("\\|.*", "", macroNames), names(macros), value = T), value = T), with = F] +
...
# get margin level from margin grwoth
forecastFrom <- 2005
forecastUpTO <- 2021

for(eu in countriesEU){
  for(loanType in names(models2varNL)){
  tmp <- copy(marginPrediction[[eu]][[loanType]])
  tmp[ ,2:5] <- tmp[, lapply(.SD, function(x){1+x/100}), .SDcols = 2:5]
  newVars <- c(paste0("marginRate_", eu, "_Baseline"), paste0("marginRate_", eu, "_Adverse"),
             paste0("marginRate_", eu, "_optimal"), paste0("marginRate_", eu, "_extreme"))
  tmp[TIME %in% marginNL[,TIME], (newVars) := marginNL[, get(loanType)]]
     for(i in forecastFrom:forecastUpTO-1 ){
      tmp[TIME > i & TIME <= i+1, (newVars) := tmp[TIME > i & TIME <= i+1,grep("margin_gr", names(tmp)), with = F] *
      tmp[TIME > i-1 & TIME <= i, grep("marginRate", names(tmp)), with = F]]
     }
  marginPrediction[[eu]][[loanType]] <- tmp
  }
}
...
for(eu in countriesEU){
  for (loanType in names(models2varNL)) {
    mar <- marginPrediction[[eu]][[loanType]][TIME > 2000 , grep("TIME|margin_gr", names(marginPrediction[[eu]][[loanType]])), with = F] 
    tmp <- get(paste0("margin_gr_",eu))
