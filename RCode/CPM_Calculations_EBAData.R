...
#load('~/Dropbox (OSIS)/142 Metis/InputData/InputFinal/Inputs.RData')
load('~/Dropbox (OSIS)/142 Metis/InputData/InputFinal/PreprocessedEBAData_withInputs.RData')

###Add loan level data
#ING CRE from addLoanLevelPortfolio_INGCRE.R
DT_all <- copy(DT)
load('~/Dropbox (OSIS)/142 Metis/InputData/InputFinal/PreprocessedLLD_CREING.RData')
DT <- rbind(DT_all, DT)
rm(DT_all)
...
#focus on core banks
DT <- DT[BankNameShort %in% CoreBanks, ]

#Align New Production inputs with current portfolio
DT[, New_Rating3 :=Rating3]
DT[, New_Rating4 :=Rating4]
DT[, New_Rating5 :=Rating5]
DT[, New_Rating6 :=Rating6]
DT[, New_Rating7 :=Rating7]
DT[, New_Rating8 :=Rating8]
DT[, New_Default :=0.0] #no defaulters initially 

DT[, SumRatings := New_Rating3 + New_Rating4 + New_Rating5 + New_Rating6 + New_Rating7 + New_Rating8]
DT[, New_Rating3 := round(New_Rating3/SumRatings,4)]
DT[, New_Rating4 := round(New_Rating4/SumRatings,4)]
DT[, New_Rating5 := round(New_Rating5/SumRatings,4)]
DT[, New_Rating6 := round(New_Rating6/SumRatings,4)]
DT[, New_Rating7 := round(New_Rating7/SumRatings,4)]
DT[, New_Rating8 := round(New_Rating8/SumRatings,4)]
DT[, New_Default := 1L - (New_Rating3 + New_Rating4 + New_Rating5 + New_Rating6 + New_Rating7 + New_Rating8)]

DT[, downturnlgd := pmax(downturnlgd,lgd)] #ensure economic lgd is not larger than downturnlgd

DT[, New_downturnlgd := downturnlgd]
DT[, New_lgd := lgd]

DT[, New_Maturity_numeric := Maturity_numeric + 1] #assume new production is slightly longer than current

###Create future replines
NewProdFrequency <- "Annual" #Annual
...
if(NewProdFrequency=="Annual"){
  ...
  DT[DateYQ>PoolCutoffDate, New_ExpFactor := (1 + New_ExposureGrowthRate)^index - (1 + New_ExposureGrowthRate)^(index-1) ] 
} 
# else { #Quarterly production
#   DT <- DT[rep(seq_len(nrow(DT)), NumForecastQtr),] # EXPAND
#   setkey(DT, ReplineID)
#   DT[, index := seq_len(.N), by = ReplineID]
#   DT[, DateYQ := as.numeric(PoolCutoffDate) + (index * (1/4)) - 1/4] 
#   DT[, DateYQ := as.yearqtr(DateYQ)]
#   DT[, index := index - 1L ] #start quarter is quarter 0 
#   DT[DateYQ>PoolCutoffDate, New_ExpFactor := (1 + New_ExposureGrowthRate/4)^index - (1 + New_ExposureGrowthRate/4)^(index-1) ]  #devide growth rate by 4 for quarterly vintages
# }
...
#DT_input <- DT[PoolCutoffDate==poolDate, .(LEI_Code, BankNameShort, Country, ReplineID, ExposureReduced, lgd, downturnlgd, Rating3, Rating4, Rating5, Rating6, Rating7, Rating8, #Default from NPL disclosure
...
#DT_input <- DT_input[BankNameShort %in% CoreBanks, ]
#
...
countries <- c("FR", "IT", "ES", "SE", "DE", "NL", "US", "UK")
...
ttc_TransitionMatrix_Data[['S1']][['LGDCorr']] <- 0.02
...
ttc_TransitionMatrix_Data[['S2']][['LGDCorr']] <- 0.02
...
BankList <- unique(DT$BankNameShort)
SelectedBank <- select.list(BankList, multiple = F, graphics = T)
DT <- DT[BankNameShort==SelectedBank,]
DTL <- DTL[BankNameShort==SelectedBank,]
...
runTime <- system.time({
  ...
  results[ AssetClass %in% c( "Corporates", "CRE"), 
           AssetCorrelation := ttc_TransitionMatrix_Data[['S1']][['AssetCorr']]]
  results[ AssetClass %in% c( "Corporates", "CRE"), 
           LGDCorr := ttc_TransitionMatrix_Data[['S1']][['LGDCorr']]]
  results[ AssetClass %in% c("Retail - SME", "residential"), 
           AssetCorrelation := ttc_TransitionMatrix_Data[['S2']][['AssetCorr']]]
  results[ AssetClass %in% c("Retail - SME", "residential"), 
           LGDCorr := ttc_TransitionMatrix_Data[['S2']][['LGDCorr']]]
  
})

save(results, file=paste0("RData/", SelectedBank, "_EBAData2017Q2_Results.RData"))
