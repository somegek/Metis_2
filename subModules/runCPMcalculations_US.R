runCPMcalculations_US <- function(DT, macroDataTable, workingDirectory){
  ...
  DT[, Maturity_numeric := Maturity_Numeric]    
  DT[, New_Maturity_numeric := New_Maturity_Numeric]    
  ...
  DT[AssetClass %in% c("Corporates - SME_", "Corporates_","LoansCommercialIndustrial","LoansRRE","Retail - Secured by real estate property_"), AssetClass := "Corporates"] #Map LoansRRE to Corporates
  DT[AssetClass %in% c("Corporates - Specialised Lending_","LoansCRE","LoansHVCRE"), AssetClass := "CRE"]
  DT[AssetClass %in% c("Retail - Other Retail - NON SME_", "Retail - Other Retail - SME_", "Retail - Qualifying Revolving_", "Retail_","LoansConsumer", "LoansOtherRetail"), AssetClass := "Retail"]
  DT[ AssetClass %in% c( "Corporates", "CRE"), Segment := "S1"]
  DT[ AssetClass %in% c("residential","Retail"), Segment := "S2"]
  DT[ ,countryMacro := "US"]

  DTL[AssetClass %in% c("Corporates - SME_", "Corporates_","LoansCommercialIndustrial","LoansRRE","Retail - Secured by real estate property_"), AssetClass := "Corporates"]
  DTL[AssetClass %in% c("Corporates - Specialised Lending_","LoansCRE","LoansHVCRE"), AssetClass := "CRE"]
  DTL[AssetClass %in% c("Retail - Other Retail - NON SME_", "Retail - Other Retail - SME_", "Retail - Qualifying Revolving_", "Retail_","LoansConsumer", "LoansOtherRetail"), AssetClass := "Retail"]
  DTL[ AssetClass %in% c( "Corporates", "CRE"), Segment := "S1"]
  DTL[ AssetClass %in% c("residential","Retail"), Segment := "S2"]
  DTL[ ,countryMacro := "US"]
